...
export const useLayerStore = create<LayerStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    // FIXED: Added safety check for window.ctg
    loadFromPersistence: async (includeImported = false) => {
      set({ isLoading: true })
      try {
        // Check if window.ctg is available
        if (!window.ctg?.layers) {
          console.warn('window.ctg not ready yet, waiting...')
          
          // Wait for window.ctg to be ready (retry up to 20 times with 100ms delay)
          let retries = 0
          while (!window.ctg?.layers && retries < 20) {
            await new Promise(resolve => setTimeout(resolve, 100))
            retries++
          }
          
          // If still not available after retries, log error and return
          if (!window.ctg?.layers) {
            console.error('window.ctg not available after retries, skipping persistence load')
            set({ isLoading: false })
            return
          }
        }
